...
document.body.addEventListener('click', e => {
  let target = e.target;
  if (target.dataset.faqToggler) faqInit(target);
...
if (isExist('.projects__swiper')) {
  const p_swiper = new swiper__WEBPACK_IMPORTED_MODULE_0__["default"]('.projects__swiper', {
    slidesPerView: 4.5,
    spaceBetween: 20,
    breakpoints: {
      320: {
...
if (isExist('.reviews__swiper')) {
  const p_swiper = new swiper__WEBPACK_IMPORTED_MODULE_0__["default"]('.reviews__swiper', {
    slidesPerView: 3,
    spaceBetween: 10,
    breakpoints: {
      320: {
...
if (isExist('.articles__swiper')) {
  const p_swiper = new swiper__WEBPACK_IMPORTED_MODULE_0__["default"]('.articles__swiper', {
    slidesPerView: 3.5,
    spaceBetween: 20,
    breakpoints: {
      320: {
...
window.addEventListener('scroll', e => {
  ...
  if (scrollY >= 200) {
    header.style.backgroundColor = '#1b1b1c';
  } else {
    header.style.backgroundColor = 'transparent';
  }
});
function rearrangeVideo() {
  const title = document.querySelector('.futures__title');
  const video = document.querySelector('.futures__video');
  const items = document.querySelector('.futures__items');
  const content = document.querySelector('.futures__content');
  if (window.innerWidth <= 1200) {
    if (title.nextElementSibling !== video) {
      content.insertBefore(video, items);
    }
  } else {
    if (video.parentNode === content && video !== content.lastElementChild) {
      content.appendChild(video);
    }
  }
}
window.addEventListener('load', rearrangeVideo);
window.addEventListener('resize', rearrangeVideo);
(function () {
